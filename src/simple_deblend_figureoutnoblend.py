...
from astrobase.lcmath import phase_magseries_with_errs, phase_bin_magseries_with_errs


def phasefold(t,m,e,per,epoch=2059.54,binsize=0.025):
    phased = phase_magseries_with_errs(t,m,e,per,epoch,wrap=True,sort=True)
    return (phased,
            phase_bin_magseries_with_errs(phased['phase'],phased['mags'],
                                          phased['errs'],binsize=binsize)
            )
